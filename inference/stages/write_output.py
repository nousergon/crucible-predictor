...
# (url_path="predictor") and guarded by that repo's slug-drift test; the page
# honors ?date=YYYY-MM-DD keyed by this run's date_str, so the email link opens
# the exact day it describes. The base URL is built by the lifted
# krepis.console.console_url chokepoint (config#1300) — only the slug (the
# cross-repo contract with the dashboard url_path) stays local, mirroring
# training/model_zoo.py's MODEL_ZOO_SLUG usage.
...
def _build_predictor_email(
    ...
    As of config#856 (pipeline-reporting revamp, "pull-for-state console page +
    push-on-transition emails") this is a **slim link email**: the body carries
    the at-a-glance summary (model/IC, UP/DOWN/veto counts, market regime, any
    unscored-buy-candidate warning) plus a deep-link to the full morning
    briefing on the console Predictor page (crucible-dashboard
    ``views/7_Predictor.py``), which renders the complete
    ``predictor/predictions/{date}.json`` artifact — the full per-ticker
    prediction table (incl. the authoritative gbm_veto column, config#1815),
    research brief, and effective optimizer params live there now, not inlined
    in the email.

    ``write_predictions`` still writes the complete predictions payload to S3
    ...
    # predictor_gbm_veto override). A display rule that diverges from gbm_veto
    # is the exact defect config#1815 fixed (2026-07-06: the email claimed
    # "14 vetoes" while gbm_veto was false for all 28 names). The per-ticker
    # veto badge now lives on the console Predictor page (single-sourced from
    # this same boolean); the email surfaces only the count.
    ...
    # ── Subject (unchanged shape — regime/counts at a glance) ────────────────
    ...
    html_body = (
        ...
        f'<p style="font-size:11px;color:#888;">Full per-ticker predictions (incl. the '
        f'gbm_veto column), research brief, and effective optimizer params are on the '
        f'console report.</p>'
    ...
    plain_body = (
        ...
        f"Model: {model_version}  {ic_label}: {ic_str}  Mode: {metrics.get('inference_mode', 'mse')}  "
...
def run(ctx: PipelineContext) -> None:
    ...
    # ── Alpha dead-band veto shadow soak (config#2297) ──────────────────────
    # Brian's ruling (2026-07-13): "add a dead-band (predicted_alpha < -epsilon),
    # observe-mode first" — log what the dead-band decision WOULD have been
    # WITHOUT changing the live gbm_veto decision above. The dead-band suppresses
    # the veto when |predicted_alpha| is below epsilon (noise-level signal).
    # Incident: veto fired on noise with median predicted_alpha ≈ -0.0004, ~half
    # the universe landing negative and getting vetoed despite low confidence
    # (e.g. IBKR -0.04%, COIN -0.07%) while high-research names got blocked.
    # Default OFF (``veto_dead_band_observe_enabled``). When enabled, reads
    # ``alpha_veto_dead_band`` epsilon from predictor_params.json (e.g. -0.001).
    # Observation artifacts: ``gbm_veto_shadow_dead_band`` (boolean — what WOULD
    # veto) and ``alpha_veto_dead_band_epsilon`` (the epsilon value) on each
    # prediction. After the observe period, a separate cutover decision promotes
    # the shadow to live. Re-validates epsilon choice against backtest evidence
    # (veto_value.json, 21d, n=668 in the issue).
    params = _load_predictor_params_from_s3(ctx.bucket) or {}
    if bool(params.get("veto_dead_band_observe_enabled", False)):
        epsilon = float(params.get("alpha_veto_dead_band", -0.001))
        n_shadow_vetoes = 0
        n_shadow_differ = 0
        for p in ctx.predictions:
            cr = p.get("combined_rank")
            alpha = p.get("predicted_alpha", 0) or 0
            confidence = p.get("prediction_confidence", 0) or 0
            live_veto = p.get("gbm_veto", False)
            shadow_veto = (
                alpha < epsilon
                and cr is not None
                and cr > n_preds / 2
            ...
            p["gbm_veto_shadow_dead_band"] = shadow_veto
            p["alpha_veto_dead_band_epsilon"] = epsilon
            if shadow_veto:
                n_shadow_vetoes += 1
            if shadow_veto != live_veto:
                n_shadow_differ += 1
        if n_shadow_differ > 0:
            log.info(
                "Alpha dead-band shadow (config#2297): epsilon=%.6f — "
                "%d live vetoes, %d shadow vetoes, %d differ",
                epsilon, sum(1 for p in ctx.predictions if p.get("gbm_veto")),
                n_shadow_vetoes, n_shadow_differ,
            )
